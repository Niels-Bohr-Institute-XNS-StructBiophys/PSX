...
def CalculateExchangeRatesForASingleChain(Chain, Temperature, pH, ReferenceData):
	...
	LambdaProtonatedAcidHis = math.log(10.0 ** (-0.80 - pH) / (10.0 ** -pKHis + 10.0 ** -pH) + 10.0 ** ( 0.00 - pKHis) / (10.0 ** -pKHis + 10.0 ** -pH), 10.0)

	RhoProtonatedAcidAsp    = math.log(10.0 ** (-0.12 - pH) / (10.0 ** -pKAsp + 10.0 ** -pH) + 10.0 ** ( 0.58 - pKAsp) / (10.0 ** -pKAsp + 10.0 ** -pH), 10.0)
	RhoProtonatedAcidGlu    = math.log(10.0 ** (-0.27 - pH) / (10.0 ** -pKGlu + 10.0 ** -pH) + 10.0 ** ( 0.31 - pKGlu) / (10.0 ** -pKGlu + 10.0 ** -pH), 10.0)
	RhoProtonatedAcidHis    = math.log(10.0 ** (-0.51 - pH) / (10.0 ** -pKHis + 10.0 ** -pH) + 10.0 ** ( 0.00 - pKHis) / (10.0 ** -pKHis + 10.0 ** -pH), 10.0)

	LambdaProtonatedBaseAsp = math.log(10.0 ** ( 0.69 - pH) / (10.0 ** -pKAsp + 10.0 ** -pH) + 10.0 ** ( 0.10 - pKAsp) / (10.0 ** -pKAsp + 10.0 ** -pH), 10.0)
	LambdaProtonatedBaseGlu = math.log(10.0 ** ( 0.24 - pH) / (10.0 ** -pKGlu + 10.0 ** -pH) + 10.0 ** (-0.11 - pKGlu) / (10.0 ** -pKGlu + 10.0 ** -pH), 10.0)
	LambdaProtonatedBaseHis = math.log(10.0 ** ( 0.80 - pH) / (10.0 ** -pKHis + 10.0 ** -pH) + 10.0 ** (-0.10 - pKHis) / (10.0 ** -pKHis + 10.0 ** -pH), 10.0)

	RhoProtonatedBaseAsp    = math.log(10.0 ** ( 0.60 - pH) / (10.0 ** -pKAsp + 10.0 ** -pH) + 10.0 ** (-0.18 - pKAsp) / (10.0 ** -pKAsp + 10.0 ** -pH), 10.0)
	RhoProtonatedBaseGlu    = math.log(10.0 ** ( 0.39 - pH) / (10.0 ** -pKGlu + 10.0 ** -pH) + 10.0 ** (-0.15 - pKGlu) / (10.0 ** -pKGlu + 10.0 ** -pH), 10.0)
	RhoProtonatedBaseHis    = math.log(10.0 ** ( 0.83 - pH) / (10.0 ** -pKHis + 10.0 ** -pH) + 10.0 ** ( 0.14 - pKHis) / (10.0 ** -pKHis + 10.0 ** -pH), 10.0)

	# Termini
	RhoAcidNTerm    = -1.32
	LambdaAcidCTerm = math.log(10.0 ** (0.05 - pH) / (10.0 ** -pKGlu + 10.0 ** -pH) + 10.0 ** (0.96 - pKGlu) / (10.0 ** -pKGlu + 10.0 ** -pH), 10.0)
...
#######################################################################
# Calculate intrinsic exchange rates and assign them to the b-factors #
#######################################################################
def CalculateExchangeRates(ProteinModel, Temperature, pH, ReferenceData):
	...
	for Chain in ProteinModel:
		...
		if not Sequence == []:
			IntrinsicEnchangeRates = CalculateExchangeRatesForASingleChain(Sequence, Temperature, pH, ReferenceData)
